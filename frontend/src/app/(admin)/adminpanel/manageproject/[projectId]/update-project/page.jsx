      ...
      if (projectData) {
        try {
          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/single-project/${projectId}`
          );
          ...
          setFormData({
            ...
            briefAboutWebsite: project.briefAboutWebsite || "",
            detailedDescription: project.detailedDescription || "",
            mainImgUrl: project.mainImgUrl || "",
            toolImgs: project.toolImgs || [],
            gellaryImgs: project.gellaryImgs || [],
            reviewerImgUrl: project.reviewerImgUrl || "",
    ...
    <div className="m-2 md:m-5">
      <div className="h-[55px] rounded-md flex items-center justify-between  gap-2 px-5 text-white  bg-[#035635] text-[1.2rem] ">
        <div className=" flex items-center gap-2 ">
          <MdAssignmentAdd />
          <div className=" font-bold">Oppdater prosjekt</div>
        </div>
        <Link
          href={`${
            queryValue === "update-details"
              ? `/adminpanel/manageproject/${projectId}/update-project`
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/update-project/${projectId}`,
        {
          method: "PUT",
